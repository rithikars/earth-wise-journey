...
create table if not exists public.eco_point_events (
...
create policy if not exists "Select own lesson_progress"
  on public.lesson_progress for select using (auth.uid() = user_id);

create policy if not exists "Insert own lesson_progress"
  on public.lesson_progress for insert with check (auth.uid() = user_id);

create policy if not exists "Update own lesson_progress"
...
create policy if not exists "Select own eco_point_events"
...
-- Helpful indexes
create index if not exists idx_eco_events_user on public.eco_point_events(user_id);
create index if not exists idx_eco_events_user_lesson on public.eco_point_events(user_id, lesson_id);
