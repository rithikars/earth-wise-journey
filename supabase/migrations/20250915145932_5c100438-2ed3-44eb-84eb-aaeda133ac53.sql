-- Drop existing functions with parameter name conflicts
DROP FUNCTION IF EXISTS public.award_quiz_points(text, integer, integer);
DROP FUNCTION IF EXISTS public.award_video_points(text);
DROP FUNCTION IF EXISTS public.award_task_points(text);

-- Recreate functions with unambiguous parameter names
